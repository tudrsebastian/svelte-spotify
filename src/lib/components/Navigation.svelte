...
<div class="nav-content" class:desktop class:mobile={!desktop}>
    {#if !desktop && isMobileMenuOpen}
    <div class="overlay" on:click={closeMenu} on:keyup={handleEscape}  transition:fade={{duration:200}}/>
    {/if}
<nav aria-label="Main">
    {#if !desktop}
    <IconButton icon={Menu} class="menu-button" label="open-menu" bind:this={openMenuButton} on:click={openMenu} aria-expanded={isOpen} />
    {/if}
    <div class="nav-content-inner" on:keyup={handleEscape} class:is-hidden={!isOpen} style:visibility={isOpen ? 'visible' : 'hidden'}>
        {#if !desktop}
